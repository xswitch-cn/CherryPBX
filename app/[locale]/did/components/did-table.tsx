...
import { z } from "zod";
import { hotlinesApi } from "@/lib/api-client";
...
export function DidTable({
  ...
  const [selectedDid, setSelectedDid] = React.useState<any>(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      const response = (await hotlinesApi.delete(selectedDid.id)) as any;
      if (response?.data?.success || response?.data?.message == "success") {
        toast.success(ttt("deleteSuccess") || "删除成功");
        const updatedData = data.filter((did) => did.id !== selectedDid.id);
        setData(updatedData);
        if (onDataChange) {
          onDataChange();
        }
  ...
  const columns = React.useMemo<ColumnDef<z.infer<typeof didSchema>>[]>(
    () => [
      ...
      },
      {
        accessorKey: "numbers",
        header: t("extension"),
        cell: ({ row }) => <span>{row.original.numbers}</span>,
      },
      {
            ...
            <DropdownMenuContent align="end" className="w-40">
              <DropdownMenuItem
                onClick={() => {
                  ...
                  window.location.href = `/${locale}/did/${row.original.id}`;
                }}
              >
                {tt("details")}
              </DropdownMenuItem>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <DeleteConfirmDialog
        ...
        description={t("deleteItem", { item: selectedDid?.line_number })}
